...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_sources_sensor_type_id", "sources", ["sensor_type_id"], unique=False
    )
    ...
    op.create_index(
        "idx_attacks_occurred_at",
        "attacks",
        [sa.literal_column("occurred_at DESC")],
        unique=False,
    )
    op.create_index(
        "idx_attacks_pending_occurred_at",
        "attacks",
        ["occurred_at", "id"],
        unique=False,
    ...
    op.create_index(
        "idx_attacks_source_occurred_at",
        "attacks",
        ["source_id", sa.literal_column("occurred_at DESC")],
        unique=False,
    )
    op.create_index(
        "idx_attacks_type_occurred_at",
        "attacks",
        ["attack_type", sa.literal_column("occurred_at DESC")],
        unique=False,
        postgresql_where=sa.text("attack_type IS NOT NULL"),
    )
    op.create_table(
        ...
        sa.Column("last_seen_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column(
            "hit_count", sa.Integer(), server_default=sa.text("1"), nullable=False
        ),
        sa.ForeignKeyConstraint(
            ["alert_id"], ["common_ip_alerts.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["source_id"],
            ["sources.id"],
