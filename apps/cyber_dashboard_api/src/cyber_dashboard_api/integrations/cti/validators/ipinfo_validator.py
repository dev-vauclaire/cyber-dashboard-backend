...
class IpinfoValidator(BaseCtiValidator):
    """Valide les credentials IPinfo."""

    provider_label = "IPinfo"

    def __init__(self, client: IpinfoClient) -> None:
        self._client = client

    def validate(self, context: CtiValidationContext) -> ValidationResult:
        if context.api_key is None:
            return ValidationResult.fail("La clé API IPinfo est manquante")
